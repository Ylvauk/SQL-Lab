...
SELECT players.name, teams.name FROM players JOIN teams ON players.team_id = teams.id WHERE teams.name = 'Buffalo Bills';
...
SELECT players.name, players.salary FROM players JOIN TEAMS ON players.team_id = teams.id WHERE teams.name = 'Green Bay Packers' ORDER BY salary ASC LIMIT 1;
